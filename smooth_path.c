 ...
 */
 ...
 // FIX PRINTING ISSUE by pass in point from the bezier curve for waypoint
...
#define K_MAX (1.0/20) // K = 1/(radius), this will be defined for the specific UAV AND KEEP THE 1.0
#define C_1 (7.2364)
...
Vector* cross_prod_vector_unit(Vector* a, Vector* b);
...
Vector smooth(Vector* a, Vector* b, Vector* c);
...
float angle_point(Point* a, Point* b) {
  return (float)acos((dot_product_point(a,b))/(norm_point(a) * norm_point(b)));
}
...
float** inverse(float** mat) {
    ...
    for (int row = 0; row < 4; row++) {
        for (int col = 0; col < 4; col++) {
            ...
            if (row == 0) {
                det +=  mat[row][col] * val * pow(-1,col+row);
            }
 ...
 * calculations, convert back to 3-D and then print to output file and return
 * a vector that will be used for creating the next waypoint
 */
Vector smooth(Vector* w_1, Vector* w_2, Vector* w_3) {
    ...
    if (fabs(u_t.x + u_p.x) > .01 ||  fabs(u_t.y + u_p.y) > .01
      ...
      float** tm = malloc(4 * sizeof(float *));
      for (int i = 0; i < 4; i++) {
        tm[i] = malloc(4 * sizeof(float));
      }
      ...
      float** w_1_4 = malloc(4 * sizeof(float *));
      float** w_2_4 = malloc(4 * sizeof(float *));
      float** w_3_4 = malloc(4 * sizeof(float *));
      for (int i = 0; i < 4; i++) {
        w_1_4[i] = malloc(sizeof(float));
        w_2_4[i] = malloc(sizeof(float));
        w_3_4[i] = malloc(sizeof(float));
      }
      ...
      w_2_4[2][0] = w_2->z;
      w_3_4[2][0] = w_3->z;
      w_1_4[3][0] = w_2_4[3][0] = w_3_4[3][0] = 1;
...
int main(void) {
  ...
  Vector w_close = smooth(&(waypoints_in[0]), &(waypoints_in[1]), &(waypoints_in[2]));

  // Read in one line and shift the array so that it is three points
  while (feof(data_read) == 0) {
    // Shift array and make use of w_close from the bezier curve to pervent backwards movement
    waypoints_in[0] = w_close;
