...
#define RESOLUTION_X 31 //keep square for 2d interpolation MUST BE EVEN
#define RESOLUTION_Y 31 //keep square for 2d interpolation MUST BE EVEN
#define RESOLUTION_Z 13 //keep unit for 2d interpolation MUST BE EVEN
...
// double gps2grid(double *gps){
// 	int grid_loc[3] = {gps[0]/x_loc_increment+offset_x,gps[1]/y_loc_increment+offset_y,
// 		gps[2]/z_loc_increment+offset_z};
// 	return grid_loc;
// }
...
// 3 - calculate location increment
	...
	int x_n_nominal = dx > 0 ? 1 : 0;
	int y_n_nominal = dy > 0 ? 1 : 0;
	int z_n_nominal = dz > 0 ? 1 : 0;
	double x_dim_size = MIN_DIM_SIZE;
	double y_dim_size = MIN_DIM_SIZE;
	double z_dim_size = MIN_DIM_SIZE;
	if(dx >= x_dim_size){
		x_dim_size += buffer_percent*dx;
	}
	if(dy >= y_dim_size){
		y_dim_size += buffer_percent*dy;
	}
	if(dz >= z_dim_size){
		z_dim_size += buffer_percent*dz;
	}

	if(dx >= x_dim_size/RESOLUTION_X){
		x_n_nominal = RESOLUTION_X*dx/x_dim_size;
		if((RESOLUTION_X-x_n_nominal)%2!=0){
			x_n_nominal --;
		}
	}
	x_loc_increment = dx > 0 ? dx/x_n_nominal : x_dim_size/RESOLUTION_X;

	if(dy >= y_dim_size/RESOLUTION_Y){
		y_n_nominal = RESOLUTION_Y*dy/y_dim_size;
		if((RESOLUTION_Y-y_n_nominal)%2!=0){
			y_n_nominal --;
		}
	}
	y_loc_increment = dy > 0 ? dy/y_n_nominal : y_dim_size/RESOLUTION_Y;

	if(dz >= z_dim_size/RESOLUTION_Z){
		z_n_nominal = RESOLUTION_Z*dz/z_dim_size;
		if((RESOLUTION_Z-z_n_nominal)%2!=0){
			z_n_nominal --;
		}
	}
	z_loc_increment = dz > 0 ? dz/z_n_nominal : z_dim_size/RESOLUTION_Z;
...
void extract_path(){
	...
	while(!nodes_equal(step_node,goal_node)){
		...
		sprintf(buff,"%lf %lf %lf\n",smallest_rhs[0]*x_loc_increment+current_location[0],smallest_rhs[1]*y_loc_increment+current_location[1],smallest_rhs[2]*z_loc_increment+current_location[0]);
		fputs(buff,sp_temp);
		clear(buff,size);
	}
	printf("step: [%d,%d,%d]\n",step_node->cell[0],step_node->cell[1],step_node->cell[2]);
...
void block_or_unblock(double *object,int infinite){
	...
	for(i = i_min; i <= i_max; i++){
		for(j = j_min; j <= j_max; j++){
			for(k = k_min; k <= k_max; k++){
				total--;
				current_radius = sqrt(pow((x_trans - (i-start_cell[0])*x_loc_increment),2) + 
									  pow((y_trans - (j-start_cell[1])*y_loc_increment),2) +
									  pow((z_trans - (k-start_cell[2])*z_loc_increment),2));
...
// 1: In list from last round
// 2: New obstacle
// 0: Removed from flight_information
void update_grid_with_obstacles(Obstacle **curr){
	if(curr && *curr){
		if((*curr)->mark){
			...
			temp = NULL;
			free(temp);
			update_grid_with_obstacles(curr);
		}
	}
}
...
void show_path(){
	...
	int z_min = min(start_cell[2],goal_cell[2]);
	int i,j,k,x,y,z;
	int buff_length;
	int cell_length = 3;
	char buff[8];
		...
		for(x = 0; x < 3; x++){
			...
			for(i = 0; i < RESOLUTION_X; i++){
				if(x == 0){ // Cell Border
					...
					if(grid[i][j][z_min].infinite >= 1){
						sprintf(buff," /OBST\\\0");
					}else if(grid[i][j][z_min].rhs == max_val){
						sprintf(buff,"RHS_inf\0");
					}else if(grid[i][j][z_min].seen == 1){
						sprintf(buff,"%.4lf\0",(grid[i][j][z_min].rhs));
						if(is_expanded(&(grid[i][j][z_min])))
							strcat(buff,"*");
					}else{
						sprintf(buff," \0");
					}
...
void show_rhs_csv(){
	...
	fprintf(show_rhs,"%d,%d,%d,\n,",RESOLUTION_X,RESOLUTION_Y,RESOLUTION_Z);
	for(k = 0; k < RESOLUTION_X; k++){
		for(j = RESOLUTION_Y-1; j >= 0; j--){
			for(i = 0; i < RESOLUTION_X; i++){
				if(grid[i][j][k].infinite == 1){
					fprintf(show_rhs,"O,");
				}else if(grid[i][j][k].rhs == max_val){
					fprintf(show_rhs,"RHS_inf,");
				}else if(grid[i][j][k].rhs == 0){
					fprintf(show_rhs," ,");
				}else{
					fprintf(show_rhs,"%.4lf,",(grid[i][j][z].rhs));
				}
			}
			fprintf(show_rhs,"\n,");
		}
		...
		for(x = 0; x < 7; x++){
			...
			for(i = 0; i < RESOLUTION_X; i++){
				if(x == 0){ // Cell Border
					...
					sprintf(buff,"Seen %d\0",(grid[i][j][z_min].seen));
					buff_length = strlen(buff);
					for(y = buff_length; y < cell_length; y++){
						strcat(buff," ");
					}
					...
					sprintf(buff,"Infinite %d\0",(grid[i][j][z_min].infinite));
					buff_length = strlen(buff);
					for(y = buff_length; y < cell_length; y++){
						strcat(buff," ");
					}
					...
					if(grid[i][j][z_min].g == max_val){
						sprintf(buff,"G inf\0");
					}else{
						sprintf(buff,"G %.4lf\0",(grid[i][j][z_min].g));						
					}
					buff_length = strlen(buff);
					for(y = buff_length; y < cell_length; y++){
						strcat(buff," ");
					...
					if(grid[i][j][z_min].rhs == max_val){
						sprintf(buff,"RHS inf\0");
					}else{
						sprintf(buff,"RHS %.4lf\0",(grid[i][j][z_min].rhs));
					}
					buff_length = strlen(buff);
					for(y = buff_length; y < cell_length; y++){
						strcat(buff," ");
					...
					sprintf(buff,"[%0.1lf,%0.1lf]\0",(grid[i][j][z_min].k[0]),(grid[i][j][z_min].k[1]));
					buff_length = strlen(buff);
					for(y = buff_length; y < cell_length; y++){
						strcat(buff," ");
					}
